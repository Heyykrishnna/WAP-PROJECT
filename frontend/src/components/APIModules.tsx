    ...
    <section className="py-20 border-t border-stone-200">
      <div className="max-w-6xl mx-auto px-8">
        <div className="flex items-end justify-between mb-12">
          <div>
            <p className="text-[10px] font-semibold tracking-widest uppercase text-stone-400 mb-3">Intelligence Modules</p>
            <h2 style={{ fontFamily: "'Doto', monospace" }} className="text-[38px] font-bold text-stone-900 leading-[1.1]">
        ...
        <div className="flex flex-wrap" style={{ margin: '-10px' }}>
          {modules.map((mod) => (
            <div key={mod.slug} className="w-1/3 p-[10px]">
              <Link to={`/apis/${mod.slug}`} className="block h-full bg-white border border-stone-200 rounded-2xl p-7 hover:border-stone-300 hover:-translate-y-1 transition-all duration-300 group">
                <div className="flex items-start justify-between mb-6">
                  <div className="w-8 h-8 rounded-xl bg-stone-100 group-hover:bg-stone-900 transition-colors duration-300" />
                  <span className={`text-[10px] font-semibold tracking-[0.08em] uppercase px-2.5 py-1 rounded-full border ${mod.status === 'Active' ? 'border-stone-200 text-stone-500 bg-stone-50' : 'border-amber-200 text-amber-600 bg-amber-50'}`}>
                    {mod.status}
                  </span>
                </div>
                <h3 className="text-[15px] font-semibold text-stone-900 mb-2">{mod.name}</h3>
                <p className="text-[13px] text-stone-400 leading-relaxed">{mod.desc}</p>
              </Link>
            </div>
          ))}
        </div>
